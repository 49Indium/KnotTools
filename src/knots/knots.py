...
class Polynomial:
    ...
    def add(self, p: Polynomial):
        ...
        return Polynomial(min(self.lowest_power, p.lowest_power), terms)
    ...
    def multiply_by_power(self, n: int):
        return Polynomial(self.lowest_power + n, list(self.terms))
...
class Knot:
    ...
    def jones_polynomial(self) -> Polynomial:
        ...
        if not self.edges[0]:
            ...
            p = p.subtract(q.multiply_by_power(1))
            p = p.subtract(q.multiply_by_power(-1))
            return p
        ...
        while current_edge not in seen_edges:
            ...
            if current_edge == self.crossings[current_crossing].in_over:
                ...
                p = p.add(r.multiply_by_power(4))
                q = self.untwist_crossing(current_crossing).jones_polynomial()
                p = p.add(q.multiply_by_power(3))
                p = p.subtract(q.multiply_by_power(1))
                return p
            else:
                p = Polynomial(0, [])
